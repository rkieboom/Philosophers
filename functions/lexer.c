...
/*   Updated: 2022/04/21 18:08:34 by rkieboom      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static int	lexer2(char **argv)
{
	...
	temp = ft_atoi(argv[1]);
	if (temp < 1 || temp > 200)
		return (ft_write_error("Keep \
amount of philosophers between 1-200!\n", 1));
	while (i < 5)
	{
		temp = ft_atoi(argv[i]);
		if (temp < 60)
...
int	lexer(int argc, char **argv)
{
	...
	if (argc < 5 || argc > 6)
	{
		printf("usage ./philo amount_of_philos \
time_to_die time_to_eat time_to_sleep [amount_of_times_to_eat]\n");
		return (1);
	}
	while (argv[i])
	{
